...
void Render::Shader::Line::SetRendData(DATA_TYPE::VERTEX *  VtxData, int size)
{
	this->Max_Grid_Line = size;
	this->vtx = VtxData;
	Create::VertexShader(&this->VertexShader, (LPSTR)this->VS);
	Create::PixelShader(&this->PixelShader, (LPSTR)this->PS);
	Create::Declaration(&this->DeclarationData, FVF_VERTEX);
}

void Render::Shader::Line::Draw()
{
	DeviceHider::SetVertexDeclaration(this->DeclarationData.pDeclaration);

	DeviceHider::SetStreamSource(this->vtx->pVetTexBuffer, sizeof(VERTEX3D));

	this->SetDeviceShader();

	this->SetDeviceMatrix();

	DeviceHider::DrawPrimitive(D3DPT_TRIANGLELIST, this->Max_Grid_Line);
}
